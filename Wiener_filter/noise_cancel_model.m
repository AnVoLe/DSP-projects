...
tmp1=filter(1,[1 -0.8],vn); % generate 501 samples
v1n=tmp1(301:500);       %keep only the last 200 (avoids transients)
tmp2=filter(1,[1 0.6],vn);    % same for v2n
v2n=tmp2(301:500);

w0=0.05*pi;
n=[0:199]';
sn=2*sin(w0*n);       % phi is any value between 0 and 2pi, phi=0 is chosen
...
rxd=xcorr(dn,xn)/N;     %cross-correlation estimator

M=12;            % Wiener filter order
rxx=rxx(N:N+M); % extract rxx(0),rxx(1),...,rxx(M) because xcorr finds negative lags as well
...
en=dn-yn;       % en is the cleaned sn here

% plot results
subplot(3,1,1),plot(n,sn,'b',n,dn,'-r')
title(['Wiener Filter of order M=',num2str(M)])
subplot(3,1,2),plot(n,dn,'-b',n,yn,'-r',n,v1n,'-g')
axis([0 200 -5 5])
